Vue.component('comp-leaflet', {
    ...
    methods: {
        initializeWorld()
        {
            let containerDiv = this.$refs.div_leaflet_container;
            ...
            // // if worlds.length == 1, then we are in the first leafletComponent, so we launch the drawAllWorlds function, but we just need to launch this function once for
            // // all worlds
        ...
        {
            console.log("reDrawShape", serieId);
            // if (serieId == -1)
            // {
            //     for (let i = 0; i < this.series.length; i++)
            //     {
    ...
    template:'\
            <div ref="div_leaflet_container" class ="comp-leaflet-container">\
            </div>',
});